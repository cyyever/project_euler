	...
	for(i=2;i<=N;i++)
	{
		if(num_attrs[i].is_prime) //质数的真除数和肯定是1，而且不是amicable number
		{
			...
			for(k=2,j=i*2;j<=N;j+=i,k++)
			{
				...
				if(num_attrs[j].prime_divisor1==0)	//如果这个合数之前没标识过互斥的，我们开始标识
				{
			...
			while(1)
			{
				...
				if(power_prime2>N)
					break;
				num_attrs[power_prime2].sum_of_divisors=num_attrs[power_prime1].sum_of_divisors+power_prime1;
		...
		else //考虑合数的质数表示，显然它的真除数和是两个互斥的除数的除数和的乘积减去它自己
		{
			if(num_attrs[i].sum_of_divisors==0)
				num_attrs[i].sum_of_divisors=(num_attrs[num_attrs[i].prime_divisor1].sum_of_divisors+num_attrs[i].prime_divisor1)*(num_attrs[num_attrs[i].prime_divisor2].sum_of_divisors+num_attrs[i].prime_divisor2)-i;
			if(num_attrs[i].sum_of_divisors<i && num_attrs[num_attrs[i].sum_of_divisors].sum_of_divisors==i)
