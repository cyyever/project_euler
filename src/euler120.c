...
uint64_t rmax(uint64_t a) {
   ...
   * 	1.如果a%4==2，那么当数列撞到a^a时，后面的数列wrap成4a 8a
   * ...，接下去又会wrap成2a开始的数列
   * 		因此只有两种可能的数列，一个是从2a开始的，一个从4a开始的，前者最近的项是(a-4)a，后者最近的项是(a-2)a，因此取后者
   * 	2.如果a%4==0，那么当数列超过a^a时，被warp成相同的数列，因此最近的项是(a-2)a
   * 	3.如果a%4==1，那么当数列超过a^a时，被warp成a 5a 9a
   * ...的数列，接下来被wrap成4a开始的数列，接下来被warp成3a开始的数列，接下来被warp成2a开始的数列。在这四种数列中，最近的项是(a-1)*a
   * 	4.如果a%4==3，那么当数列超过a^a时，被warp成3a 7a 11a
   * ...的数列，接下来被wrap成4a开始的数列，接下来被warp成a开始的数列，接下来被warp成2a开始的数列。在这四种数列中，最近的项是(a-1)*a
