...
static uint64_t get_next_binary_palindromic_num(void);
...
static uint64_t get_next_binary_palindromic_num(void)
{
	static uint64_t *palindromic_array[2];		//奇数位数和偶数位数的回文
	uint64_t *tmp;
	...
	if(!palindromic_array[j])	
	{
		palindromic_array_len[j]=2;
		palindromic_array[j]=malloc(palindromic_array_len[j]*sizeof(uint64_t));
		if(!palindromic_array[j])
		{
	...
	tmp=realloc(palindromic_array[j],palindromic_array_len[j]*2*sizeof(uint64_t));
	if(!tmp)
	{
		printf("realloc failed:%m\n");
		exit(EXIT_FAILURE);
	}
	palindromic_array[j]=tmp;
	//先构造 1回文1
	for(i=palindromic_array_len[j];i<palindromic_array_len[j]*2;i++)
	{
		palindromic_array[j][i]=palindromic_array[j][i-palindromic_array_len[j]]<<1;
