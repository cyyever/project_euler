...
int main() {
  ...
  while (true) {
    ...
    } else if (i == 1) //遍历d7 d8
    {
      ...
      {
        for (tmp_num = divisor; tmp_num < digit_stack[i - 1] * 100;
             tmp_num += divisor)
          ;
      } else //继续上次的迭代
      {
        ...
        tmp_num = digit_stack[i - 1] * 100 + digit_stack[i] * 10 +
                  digit_stack[i + 1] + divisor;
      }
      ...
      while (tmp_num < (digit_stack[i - 1] + 1) * 100) {
        ...
        if (digit_stack[i] != digit_stack[i + 1] &&
            (num_mask & (1 << digit_stack[i])) == 0 &&
            (num_mask & (1 << digit_stack[i + 1])) == 0) {
          flag = 1;
          break;
        }
        tmp_num += divisor;
    ...
    } else if (i == 3) //处理d9 d10 因为这边必须被>10的数整除，因此只有一个可能
    {
      ...
      //先确定d9
      for (tmp_num = divisor;
           tmp_num < digit_stack[i - 2] * 100 + (digit_stack[i - 1] + 1) * 10;
           tmp_num += divisor)
        ;
      tmp_num -= divisor;
      ...
      divisor = 17;
      for (tmp_num = divisor;
           tmp_num < digit_stack[i - 1] * 100 + (digit_stack[i] + 1) * 10;
           tmp_num += divisor)
        ;
      tmp_num -= divisor;
    ...
    } else if (i == 5) //遍历d5
    {
      ...
      flag = 0;
      while (tmp_num < 10) {
        if (num_attrs[tmp_num * 100 + digit_stack[0] * 10 + digit_stack[1]]
                .is_7_dividable) {
    ...
    } else if (i == 7) //遍历d3
    {
      ...
      flag = 0;
      while (tmp_num < 10) {
        if (num_attrs[tmp_num * 100 + digit_stack[i - 1] * 10 +
                      digit_stack[i - 2]]
                .is_3_dividable) {
          if ((num_mask & (1 << tmp_num)) == 0) {
            flag = 1;
    ...
    } else if (i == 9) //遍历d1
    {
      ...
      sprintf(num_str, "%d%d%d%d%d%d%d%d%d%d", digit_stack[9], digit_stack[8],
              digit_stack[7], digit_stack[6], digit_stack[5], digit_stack[0],
              digit_stack[1], digit_stack[2], digit_stack[3], digit_stack[4]);
      sum += strtoul(num_str, nullptr, 10);
      i--;
    }
  }
  printf("%" PRIu64 "\n", sum);
