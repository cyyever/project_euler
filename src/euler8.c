...
#define DIGIT_NUM (sizeof(n)-1)
	...
	while(next_run_begin<=DIGIT_NUM-RUN_LEN)
	{
		...
		if(n[next_run_begin+RUN_LEN-1]==0)	
		{
			...
			//检查新游程是否包含0
			while(next_run_begin<=DIGIT_NUM-RUN_LEN && memchr(n+next_run_begin,0,RUN_LEN))
				next_run_begin=next_run_begin+RUN_LEN;
		...
		else
		{
			...
			if(distance<=RUN_LEN)	//两个游程有共用的数字
			{
				...
				next_cmp_product=next_cmp_product*n[next_run_begin+RUN_LEN-1]/n[next_run_begin-1];
		}
