...
static void distribute_prime_powers(uint64_t min_divisor_num)
{
	...
	for(max_prime_index=1;;max_prime_index++)
	{
		...
		//尝试从多个指数平摊
		while(prime_attrs[max_prime_index].power<prime_attrs[max_prime_index-1].power)
		{
			...
			while(i!=UINT64_MAX)
			{
				...
				while(prime_attrs[i].power>prime_attrs[max_prime_index].power && tmp*(2*(prime_attrs[i].power-1)+1)>min_divisor_num)
				{
					prime_attrs[i].power--;
