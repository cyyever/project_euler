...
int main(int argc,char **argv)
{
	...
	for(encryption_key[0]='a';encryption_key[0]<='z';encryption_key[0]++)
	{
		for(encryption_key[1]='a';encryption_key[1]<='z';encryption_key[1]++)
		{
			...
			{
				string tmp;
				for(decltype(encrypted_codes.size()) i=0;i<encrypted_codes.size();i++)
				{
					plain_char=encryption_key[i%3]^((unsigned char)encrypted_codes[i]);
					tmp.push_back(plain_char);
				}
				if(tmp.find(" the ")==string::npos && tmp.find(".The ")==string::npos && tmp.find(" The ")==string::npos)
					continue;
